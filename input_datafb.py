...
class AudioProcessor(object):
    ...
    def __init__(self, data_url, data_dir, silence_percentage, unknown_percentage,
                 ...
                 model_settings, label_smooth= False):
    ...
    def prepare_processing_graph(self, model_settings):
        ...
        # fbank_feat = logfbank(background_clamp, samplerate=16000)
    ...
    def get_data(self, how_many, offset, model_settings, background_frequency,
                 background_volume_range, time_shift, mode, sess, label_smooth=False):
        ...
        data = np.zeros((sample_count, 99*26))#model_settings['fingerprint_size']
        ...
        for i in xrange(offset, offset + sample_count):
            ...
            #11->0.008
            # rate, tmp = wav.read(sample['file'])
            tmp = logfbank(tmp, samplerate=16000)
            tmp.shape = (-1,)
            # print('input shape', tmp.shape)
            data[i - offset, :] = tmp
            label_index = self.word_to_index[sample['label']]
            if label_smooth:
                prob_eps = max( np.abs( time_shift_amount/10.0 ), 0.01)
                labels[i-offset, :] = prob_eps
                labels[i - offset, label_index] = 1 - prob_eps
            else:
