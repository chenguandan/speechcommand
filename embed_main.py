...
do_cache = True
import merge_res
def cache_embed(sess, embed1, fingerprint_input1, dropout_prob):
    ...
    def get_data(batch_size, i):
        sample_count = max(0, min(batch_size, set_size- i))
        return test_fgps[i:i+sample_count]
    embed_vecs = []
    for i in xrange(0, set_size, FLAGS.batch_size):
        test_fingerprints = get_data(FLAGS.batch_size, i)
        embed_vec = sess.run(embed1,
                       feed_dict={
                           fingerprint_input1: test_fingerprints,
                           dropout_prob: 1.0
                       }
                       )
        embed_vecs.append( embed_vec )
        if (i//FLAGS.batch_size) % 300 == 0:
            print('progress ', i, '/', set_size )
    embed_vecs = np.concatenate(embed_vecs, axis=0)
    print(embed_vecs.shape)
    np.save('out/embed_p{}.npy'.format(is_person), embed_vecs)
    # merge_res.pickle_dump(embed_vecs, 'out/embed_p{}.pkl'.format(is_person))
...
def main(_):
    ...
    if do_cache:
        cache_embed(sess, embed1, fingerprint_input1, dropout_prob)
        return
...
class ModelConfigureLace(object):
    ...
    def __init__(self):
        ...
        self.learning_rate = '0.001,0.0001'
        self.batch_size = 256
        self.start_checkpoint = 'model_lace_p{}/lace.ckpt-11000'.format(is_person)
