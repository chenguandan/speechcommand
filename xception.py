...
def create_xception_model(fingerprint_input, model_settings,
    ...
    logits, end_points = xception(fingerprint_4d, num_classes=label_count, is_training=is_training)
    if is_training:
...
def create_xception_model_p(fingerprint_input, person_embed, word_embed,model_settings,
                                   is_training, use_fb=False):
    if is_training:
        dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
    input_frequency_size = model_settings['dct_coefficient_count']
    ...
    if use_fb:
        ...
        fingerprint_4d = tf.reshape(fingerprint_input,
                                    [-1, input_time_size, input_frequency_size, 1])
    logits, end_points = xception(fingerprint_4d, num_classes=label_count, is_training=is_training, return_embed=True)
...
def xception(inputs,
            num_classes=1001,
            is_training=True,
            scope='xception', return_embed= False):
    ...
    with tf.variable_scope('Xception') as sc:
        ...
        with slim.arg_scope([slim.separable_conv2d], depth_multiplier=1),\
         ...
         slim.arg_scope([slim.batch_norm], is_training=is_training):
            ...
            shape = get_layer_shape(net)
            print('block 13 ', shape)
            ...
            shape = get_layer_shape(net)
            print('pool 14(2)', shape)
            #原本是10,10
            net = slim.avg_pool2d(net, [3,2], scope='block15_avg_pool')
            #Replace FC layer with conv layer instead
            net = slim.conv2d(net, 2048, [1,1], scope='block15_conv1')
            shape = get_layer_shape(net)
            print('pool 15', shape)
            if return_embed:
                logits = tf.squeeze(net, [1, 2], name='block15_logits')  # Squeeze height and width only
