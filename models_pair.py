...
from resnet import create_resnet_model, create_resnet_pairs_model,create_resnet_model101, create_resnetblstm_model
from xception import create_xception_model
from SE_ResNeXt import create_senet_resnet
# from dual_path_network import create_dpnet92, create_dpnet98, create_dpnet107, create_dpnet137
...
def create_model(fingerprint_input1, fingerprint_input2, model_settings, model_architecture,
    ...
    if model_architecture == 'resnet':
        ...
        #                            is_training)
        return create_resnet_pairs_model(fingerprint_input1, fingerprint_input2, model_settings,
                                  is_training)
    # elif model_architecture == 'resnet101':
    #     return create_resnet_model101(fingerprint_input, model_settings,
    #                                is_training)
    elif model_architecture == 'resnetblstm':
        return create_resnetblstm_model(fingerprint_input1, fingerprint_input2, model_settings,
                                   is_training)
    # elif model_architecture == 'xception':
    #     return create_xception_model(fingerprint_input, model_settings,
    #                                is_training)
    elif model_architecture == 'cnnblstm':
        return create_cnnblstm_model(fingerprint_input1, fingerprint_input2, model_settings,
                                     is_training)
    ...
    #                                     is_training)
        ...
        return create_lace(fingerprint_input1, fingerprint_input2, model_settings,
                                  is_training)
    # elif model_architecture == 'lace2':
    #     return create_lacev2(fingerprint_input, model_settings,
    #                                     is_training)
    # elif model_architecture == 'se_resnet':
    #     return create_senet_resnet(fingerprint_input, model_settings,
    #                         is_training)
    # elif model_architecture == 'dpnet92':
    #     return create_dpnet92(fingerprint_input, model_settings,
    #                    is_training)
    # elif model_architecture == 'dpnet98':
    #     return create_dpnet98(fingerprint_input, model_settings,
    #                           is_training)
    # elif model_architecture == 'dpnet107':
    #     return create_dpnet107(fingerprint_input, model_settings,
    #                           is_training)
    # elif model_architecture == 'dpnet137':
    #     return create_dpnet137(fingerprint_input, model_settings,
    #                           is_training)
    else:
...
def cnnblstm_embed(fingerprint_4d, is_training, dropout_prob, reuse):
    data_format = 'channels_last'
    with tf.variable_scope("cnnblstm", reuse=reuse):
        ...
        with tf.variable_scope("cell_op_1"):
            outputs, _ = tf.nn.static_bidirectional_rnn(f_cell, b_cell, conv_rd,
        ...
        out = tf.layers.dense(inputs=out, units=128, name='dense_fn')
        final_fc = tf.identity(out, 'final_dense_small')
    return final_fc


def create_cnnblstm_model(fingerprint_input1, fingerprint_input2, model_settings,
    ...
    #setting
    embed1 = cnnblstm_embed(fingerprint_4d1, is_training, dropout_prob, reuse=False)
    embed2 = cnnblstm_embed(fingerprint_4d2, is_training, dropout_prob, reuse=True)
...
def create_cnnblstm_model_v6(fingerprint_input, model_settings,
    ...
    data_format = 'channels_last'
    conv1 = conv2d_fixed_padding(
        inputs=fingerprint_4d, filters=32, kernel_size=3, strides=1,
        data_format=data_format)
    conv1 = batch_norm_relu(conv1, is_training, data_format)
    print('layer1 shape',get_layer_shape(conv1))
    # inputs = tf.identity(inputs, 'initial_conv')
    conv2 = conv2d_fixed_padding(
        inputs=conv1, filters=32, kernel_size=3, strides=1,
        data_format=data_format)
    conv2 = batch_norm_relu(conv2, is_training, data_format)
    pool2 = tf.layers.max_pooling2d(
        inputs=conv2, pool_size=2, strides=1, padding='SAME',
        data_format=data_format)
    conv3 = conv2d_fixed_padding(
        inputs=pool2, filters=32, kernel_size=3, strides=1,
        data_format=data_format)
    conv3 = batch_norm_relu(conv3, is_training, data_format)
    pool3 = tf.layers.max_pooling2d(
        inputs=conv3, pool_size=2, strides=1, padding='SAME',
        data_format=data_format)
    #TODO
    # conv_reshape = tf.squeeze(pool2, squeeze_dims=[1])
    pool3_shape = get_layer_shape(pool3)
    print('layer2 shape', pool3_shape)
    conv_reshape = tf.reshape(pool2,[-1,pool3_shape[1],pool3_shape[2]*pool3_shape[3]] )
    shape = get_layer_shape(conv_reshape)
    print("CNN --> RNN Reshape = ", shape)
    conv_rd = tf.layers.conv1d(
        inputs=conv_reshape, filters=256, kernel_size=1, strides=1,padding='same',
...
def create_cnnblstm_bnd_model_v6(fingerprint_input, model_settings,
    ...
    for layer_index in range(num_layer):
        with tf.variable_scope("cell_def_{}".format(layer_index)):
            # f_cell = tf.nn.rnn_cell.GRUCell(nb_hidden)
            # b_cell = tf.nn.rnn_cell.GRUCell(nb_hidden)
            f_cell = tf.contrib.rnn.LayerNormBasicLSTMCell(nb_hidden, dropout_keep_prob=1.0)
            b_cell = tf.contrib.rnn.LayerNormBasicLSTMCell(nb_hidden, dropout_keep_prob=1.0)
...
def batch_norm_relu_1d(inputs, is_training, data_format,name=None):
...
def conv1d_fixed_padding(inputs, filters, kernel_size, strides, data_format,name=None):
...
def block_layer_1d(inputs, filters, block_fn, blocks, strides, is_training, name,
                data_format, name_conv = None):
    # filters_out = 4 * filters if block_fn is bottleneck_block else filters
    filters_out = filters
    def projection_shortcut(inputs):
        return conv1d_fixed_padding(
            inputs=inputs, filters=filters_out, kernel_size=1, strides=strides,
            data_format=data_format, name=name_conv if name_conv is None else name_conv+'_proj')

    # Only the first block per block_layer uses projection_shortcut and strides
    inputs = block_fn(inputs, filters, is_training, projection_shortcut, strides,
                      data_format, name=name_conv+str(0))

    for bi in range(1, blocks):
        inputs = block_fn(inputs, filters, is_training, None, 1, data_format, name=name_conv+str(bi))
...
def building_block_1d(inputs, filters, is_training, projection_shortcut, strides,
    ...
    inputs = conv1d_fixed_padding(
        ...
        data_format=data_format, name=name if name is None else name+'_conv1')

    inputs = batch_norm_relu_1d(inputs, is_training, data_format,
                                name=name if name is None else name+'_bn')
    inputs = conv1d_fixed_padding(
        inputs=inputs, filters=filters, kernel_size=3, strides=1,
        data_format=data_format, name=name if name is None else name+'_conv2')

    return inputs + shortcut


def lace_embed(fingerprint_3d, is_training, dropout_prob, reuse):
    data_format = 'channels_last'
    with tf.variable_scope("lace", reuse=reuse):
        ...
        inputs = tf.layers.max_pooling1d(
            ...
            data_format=data_format, name='pool1')
        ...
        inputs = block_layer_1d(
            ...
            data_format=data_format, name_conv='block_conv1')
        inputs = block_layer_1d(
            inputs=inputs, filters=128, block_fn=block_fn, blocks=layers[1],
            strides=2, is_training=is_training, name='block_layer2',
            data_format=data_format, name_conv='block_conv2')
        inputs = block_layer_1d(
            inputs=inputs, filters=256, block_fn=block_fn, blocks=layers[2],
            strides=2, is_training=is_training, name='block_layer3',
            data_format=data_format, name_conv='block_conv3')
        inputs = block_layer_1d(
            inputs=inputs, filters=512, block_fn=block_fn, blocks=layers[3],
            strides=2, is_training=is_training, name='block_layer4',
            data_format=data_format, name_conv='block_conv4')
        ...
        inputs_ = tf.layers.average_pooling1d(
            inputs=inputs, pool_size=shape[1], strides=1, padding='VALID',
            data_format=data_format, name='pool_m1')
        ...
        inputs = tf.layers.dense(inputs=inputs_, units=512, name='dense_fn')
        final_fc = tf.identity(inputs, 'final_dense_small')
    return final_fc

def create_lace(fingerprint_input1, fingerprint_input2, model_settings,
                                  is_training, use_fb = False):
    if is_training:
        dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
    input_frequency_size = model_settings['dct_coefficient_count']
    ...
    print('shape',(input_time_size, input_frequency_size))
    if use_fb:
        fingerprint_3d1 = tf.reshape(fingerprint_input1,
                                    [-1, input_time_size+1, 26])
        fingerprint_3d2 = tf.reshape(fingerprint_input2,
                                    [-1, input_time_size+1, 26])
    else:
        fingerprint_3d1 = tf.reshape(fingerprint_input1,
                                    [-1, input_time_size, input_frequency_size])
    ...
    print((input_time_size, input_frequency_size))
    #setting
    embed1 = lace_embed(fingerprint_3d1, is_training, dropout_prob, reuse=False)
    embed2 = lace_embed(fingerprint_3d2, is_training, dropout_prob, reuse=True)
    ds = tf.layers.dense( tf.concat( [embed1*embed2, embed1+embed2, tf.abs(embed1-embed2)], axis=-1), 128 )
    ds = tf.nn.leaky_relu(ds)
    logits = tf.layers.dense( ds, 1 )
    if is_training:
        return embed1, embed2, logits, dropout_prob
    else:
        return embed1, embed2, logits
