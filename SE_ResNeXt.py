...
def Batch_Normalization(x, training, scope):
    with arg_scope([batch_norm],
        ...
        if training:
            ...
            return batch_norm(inputs=x, is_training=training, reuse=True)
...
class SE_ResNeXt():
    def __init__(self, x, class_num, training, return_embed = False):
        self.training = training
        self.return_embed = return_embed
        self.model = self.Build_SEnet(x, class_num)
    ...
    def squeeze_excitation_layer(self, input_x, out_dim, ratio, layer_name):
        with tf.name_scope(layer_name) :
            ...
            excitation = Fully_connected(squeeze, units=int(out_dim / ratio), layer_name=layer_name+'_fully_connected1')
    ...
    def Build_SEnet(self, input_x, class_num):
...
def create_senet_resnet(fingerprint_input, model_settings,
                                  is_training):
    if is_training:
...
def create_senet_resnet_p(fingerprint_input, person_embed, word_embed, model_settings,
                                  is_training):
    if is_training:
        dropout_prob = tf.placeholder(tf.float32, name='dropout_prob')
    input_frequency_size = model_settings['dct_coefficient_count']
    ...
    # training_flag = tf.placeholder(tf.bool)
    # learning_rate = tf.placeholder(tf.float32, name='learning_rate')
    logits = SE_ResNeXt(fingerprint_4d, label_count, training=is_training, return_embed=True).model
    logits = merge_ops(logits, person_embed, word_embed, is_training)
    logits = tf.layers.dense(logits, label_count)
    if is_training:
        return logits, dropout_prob
    else:
        return logits
