...
def main(_):
    ...
    # downloading.
    len(input_data_s.prepare_words_list(FLAGS.wanted_words.split(',')))
    model_settings = models.prepare_model_settings(
        2,
        FLAGS.sample_rate, FLAGS.clip_duration_ms, FLAGS.window_size_ms,
        FLAGS.window_stride_ms, FLAGS.dct_coefficient_count)
    audio_processor = input_data_s.AudioProcessor(
    ...
    with tf.name_scope('cross_entropy'):
        ...
        def hinge_loss(y, y_hat):
            return y[:,0] * tf.nn.relu(y_hat[:,1] + margin - y_hat[:,0]) + \
            y[:,1] * tf.nn.relu(y_hat[:,0] + margin-y_hat[:,1] )
        cross_entropy_mean = tf.reduce_mean( hinge_loss(ground_truth_input, logits) )
...
class ModelConfigureCNNBLSTM(object):
    ...
    def __init__(self):
        self.summaries_dir='./logs_cnnblstm_s'#retrain_logs
        ...
        self.train_dir ='./model_cnnblstm_s'
