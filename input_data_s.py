...
class AudioProcessor(object):
  ...
  def prepare_data_index(self, silence_percentage, unknown_percentage,
    ...
    for wav_path in gfile.Glob(search_path):
      ...
      if word in wanted_words_index:
        self.data_index[set_index].append({'label': 1, 'file': wav_path})
      else:
        self.data_index[set_index].append({'label': 0, 'file': wav_path})
        # unknown_index[set_index].append({'label': 0, 'file': wav_path})
    if not all_words:
  ...
  def get_data(self, how_many, offset, model_settings, background_frequency,
    ...
    for i in xrange(offset, offset + sample_count):
      ...
      label_index = sample['label']#self.word_to_index[sample['label']]
